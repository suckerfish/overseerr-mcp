...
async def request_media(
    ...
    Returns:
        Request confirmation with title, status, and request ID
    """
    try:
        client = get_client()
        ...
        # For TV shows, determine seasons to request
        season_list = None
        seasons_requested = None

        if mt == MediaType.TV:
            # Get season info for validation
            tv_seasons = [s for s in details.get("seasons", []) if s.get("seasonNumber", 0) > 0]
            ...
            elif len(tv_seasons) == 1:
                # Single season show - auto-select
                season_list = [tv_seasons[0]["seasonNumber"]]
                seasons_requested = str(tv_seasons[0]["seasonNumber"])
            else:
                # Multi-season show with no selection - reject
                raise ToolError(
                    f"This show has {len(tv_seasons)} seasons. "
